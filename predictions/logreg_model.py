#!/usr/bin/env python3
"""
Logistic Regression with Polynomial Interactions — Ensemble partner for XGBoost.

Pipeline: StandardScaler -> PolynomialFeatures(degree=2, interaction_only=True)
-> SelectKBest(f_classif, k=200) -> LogisticRegression(C=0.1, saga solver).

PolynomialFeatures on ~143 features creates ~10K interaction features.
SelectKBest prunes to top 200 to avoid memory/overfitting issues.

Reuses exact same features as XGBoost (engineer_features from xgb_model.py).

Usage:
    python3 predictions/logreg_model.py train          # Train on all data
    python3 predictions/logreg_model.py eval           # Walk-forward CV
    python3 predictions/logreg_model.py score <file>   # Score a board/results file
"""
...
MODEL_PATH = os.path.join(PREDICTIONS_DIR, 'logreg_model.pkl')
META_PATH = os.path.join(PREDICTIONS_DIR, 'logreg_model_meta.json')
...
SELECT_K = 200  # Top K interaction features to keep
...
def _build_pipeline():
    """Build the full LogReg pipeline: Scale -> Poly -> Select -> LogReg."""
    return Pipeline([
...
def walk_forward_cv(X, y, dates, sample_weights=None, sources=None):
    ...
    for i in range(1, len(graded_dates)):
        ...
        # Impute NaN
        X_train_imp, medians_fold = _impute(X_train_raw)
        X_test_imp, _ = _impute(X_test_raw, medians=medians_fold)

        # Build and train pipeline
        pipe = _build_pipeline()

        # Adjust k if fewer features after poly than SELECT_K
        # (shouldn't happen with 143 features, but be safe)
        try:
            pipe.fit(X_train_imp, y_train_raw, logreg__sample_weight=sw_train)
        except ValueError:
            # If k > n_features after poly, reduce k
            n_poly = pipe.named_steps['poly'].fit_transform(X_train_imp[:1]).shape[1]
            actual_k = min(SELECT_K, n_poly)
            pipe.named_steps['select'].k = actual_k
            pipe.fit(X_train_imp, y_train_raw, logreg__sample_weight=sw_train)

        y_prob = pipe.predict_proba(X_test_imp)[:, 1]
...
def _calibrate_prob(raw_prob, cal_map):
    """Apply calibration map to a raw logreg_prob."""
    if not cal_map:
        return raw_prob
    for b in cal_map:
...
def train_and_evaluate(use_historical=True):
    """Train LogReg pipeline on all data, run walk-forward CV, save model."""
    print("=" * 60)
    print(f"  Logistic Regression + Poly Interactions — Training {'(+historical)' if use_historical else '(graded only)'}")
    print(f"  C={C_REG}, solver={SOLVER}, poly_degree={POLY_DEGREE}, select_k={SELECT_K}")
    print("=" * 60)
    ...
    # Walk-forward CV
    print("\n  Walk-Forward Cross-Validation (LogReg + Poly):")
    ...
    if folds:
        ...
        print(f"\n  LogReg CV Summary: Avg AUC={avg_auc:.3f}, Avg Acc={avg_acc:.3f}")
    else:
        avg_auc = None
        avg_acc = None

    # Train final model on all data
    print(f"\n  Training final LogReg pipeline on {len(y)} samples...")

    # Impute
    X_imp, medians = _impute(X)

    # Build and train pipeline
    pipe = _build_pipeline()
    try:
        pipe.fit(X_imp, y, logreg__sample_weight=sample_weights)
    except ValueError:
        n_poly = pipe.named_steps['poly'].fit_transform(X_imp[:1]).shape[1]
        actual_k = min(SELECT_K, n_poly)
        pipe.named_steps['select'].k = actual_k
        pipe.fit(X_imp, y, logreg__sample_weight=sample_weights)

    # Report selected feature count
    n_poly_features = pipe.named_steps['poly'].n_output_features_
    n_selected = pipe.named_steps['select'].get_support().sum()
    print(f"  Polynomial features: {X.shape[1]} -> {n_poly_features} -> {n_selected} selected")
    ...
    print(f"  LogReg model saved: {MODEL_PATH}")
    ...
    metadata = {
        ...
        'n_features_poly': int(n_poly_features),
    ...
    print(f"\n  Training: {metadata['n_samples']} samples, {metadata['n_features_input']} input -> {metadata['n_features_selected']} selected features")
...
def score_props(results, model_path=None):
    """Load trained LogReg pipeline and add logreg_prob to each result dict."""
    if model_path is None:
        model_path = MODEL_PATH

    if not os.path.exists(model_path):
        raise FileNotFoundError(f"No trained model at {model_path}. Run: python3 logreg_model.py train")
    ...
    X_imp, _ = _impute(X, medians=medians)

    probs = pipe.predict_proba(X_imp)[:, 1]

    for r, prob in zip(results, probs):
        raw = round(float(prob), 4)
        r['logreg_prob'] = raw
        if cal_map:
            r['logreg_prob_calibrated'] = round(_calibrate_prob(raw, cal_map), 4)
...
def main():
    ...
    if command == 'train':
        ...
        print("=" * 60)
        print(f"  Logistic Regression + Poly Interactions — Evaluation Only")
        print("=" * 60)
        ...
        if len(sys.argv) < 3:
            print("Usage: python3 logreg_model.py score <board_or_results.json>")
            sys.exit(1)
        ...
        logreg_probs = [r['logreg_prob'] for r in results if 'logreg_prob' in r]
        if logreg_probs:
            arr = np.array(logreg_probs)
            print(f"  logreg_prob: mean={arr.mean():.3f}, std={arr.std():.3f}, "
