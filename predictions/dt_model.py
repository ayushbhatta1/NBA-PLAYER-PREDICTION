#!/usr/bin/env python3
"""
Decision Tree Prop Classifier — Interpretable ensemble partner for XGBoost.

sklearn DecisionTreeClassifier with StandardScaler + median imputation.
Reuses exact same features as XGBoost (engineer_features from xgb_model.py).

Hyperparameters tuned to prevent overfitting (DTs are prone to it):
  max_depth=8, min_samples_leaf=20, min_samples_split=40, max_features='sqrt'

Includes export_tree_text() for human-readable tree structure (interpretability).

Usage:
    python3 predictions/dt_model.py train          # Train on all data
    python3 predictions/dt_model.py eval           # Walk-forward CV
    python3 predictions/dt_model.py score <file>   # Score a board/results file
"""
...
MODEL_PATH = os.path.join(PREDICTIONS_DIR, 'dt_model.pkl')
META_PATH = os.path.join(PREDICTIONS_DIR, 'dt_model_meta.json')
...
# ===============================================================
# TREE EXPORT (INTERPRETABILITY)
# ===============================================================

def export_tree_text(model, max_depth=4):
    """Print a human-readable tree structure using sklearn.tree.export_text.

    Limits depth for readability. Full tree available via max_depth=None.
    """
    feature_names = list(FEATURE_COLS[:model.n_features_in_])
    # Pad if FEATURE_COLS is shorter than n_features_in_
    while len(feature_names) < model.n_features_in_:
        feature_names.append(f"f{len(feature_names)}")

    tree_str = export_text(model, feature_names=feature_names, max_depth=max_depth)
    print("\n  Decision Tree Structure (depth limited to %d):" % max_depth)
...
def walk_forward_cv(X, y, dates, sample_weights=None, sources=None):
    ...
    for i in range(1, len(graded_dates)):
        ...
        fold = {
            ...
            'tree_n_leaves': int(model.get_n_leaves()),
...
def _calibrate_prob(raw_prob, cal_map):
    """Apply calibration map to a raw dt_prob."""
    if not cal_map:
        return raw_prob
    for b in cal_map:
...
def train_and_evaluate(use_historical=True):
    """Train Decision Tree on all data, run walk-forward CV, save."""
    print("=" * 60)
    print(f"  Decision Tree — Training {'(+historical)' if use_historical else '(graded only)'}")
    print(f"  max_depth=8, min_samples_leaf=20, min_samples_split=40, max_features=sqrt")
    print("=" * 60)
    ...
    print("\n  Walk-Forward Cross-Validation (Decision Tree):")
    ...
    if folds:
        ...
        print(f"\n  DT CV Summary: Avg AUC={avg_auc:.3f}, Avg Acc={avg_acc:.3f}")
    else:
        avg_auc = None
        avg_acc = None

    # Train final model on all data
    print(f"\n  Training final Decision Tree on {len(y)} samples...")
    ...
    print(f"  Final tree: depth={model.get_depth()}, leaves={model.get_n_leaves()}")
    ...
    # Export readable tree structure (depth limited for readability)
    export_tree_text(model, max_depth=4)
    ...
    metadata = {
        ...
        'final_tree_depth': int(model.get_depth()),
        'final_tree_n_leaves': int(model.get_n_leaves()),
        'calibration_map': calibration_map,
        'model_path': MODEL_PATH,
        'architecture': 'DecisionTree(depth=8, leaf=20, split=40, feat=sqrt)',
    }
...
def score_props(results, model_path=None):
    """Load trained Decision Tree and add dt_prob to each result dict."""
    if model_path is None:
        model_path = MODEL_PATH

    if not os.path.exists(model_path):
        raise FileNotFoundError(f"No trained model at {model_path}. Run: python3 dt_model.py train")
    ...
    for r, prob in zip(results, probs):
        ...
        r['dt_prob'] = raw
        if cal_map:
            r['dt_prob_calibrated'] = round(_calibrate_prob(raw, cal_map), 4)
...
def main():
    ...
    if command == 'train':
        ...
        if metadata:
            ...
            print(f"  Final tree: depth={metadata['final_tree_depth']}, leaves={metadata['final_tree_n_leaves']}")

    elif command == 'eval':
        print("=" * 60)
        print(f"  Decision Tree — Evaluation Only")
        print("=" * 60)
        ...
        if len(sys.argv) < 3:
            print("Usage: python3 dt_model.py score <board_or_results.json>")
            sys.exit(1)
        ...
        scored = sum(1 for r in results if 'dt_prob' in r)
        print(f"  Scored {scored}/{len(results)} props")

        dt_probs = [r['dt_prob'] for r in results if 'dt_prob' in r]
        if dt_probs:
            arr = np.array(dt_probs)
            print(f"  dt_prob: mean={arr.mean():.3f}, std={arr.std():.3f}, "
