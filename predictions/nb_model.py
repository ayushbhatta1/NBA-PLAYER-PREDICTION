#!/usr/bin/env python3
"""
Naive Bayes Prop Classifier — Ensemble partner for XGBoost.

sklearn GaussianNB with StandardScaler + median imputation.
Reuses exact same features as XGBoost (engineer_features from xgb_model.py).

Usage:
    python3 predictions/nb_model.py train          # Train on all data
    python3 predictions/nb_model.py eval           # Walk-forward CV
    python3 predictions/nb_model.py score <file>   # Score a board/results file
"""
...
MODEL_PATH = os.path.join(PREDICTIONS_DIR, 'nb_model.pkl')
META_PATH = os.path.join(PREDICTIONS_DIR, 'nb_model_meta.json')
...
def walk_forward_cv(X, y, dates, sources=None):
...
def _calibrate_prob(raw_prob, cal_map):
    """Apply calibration map to a raw nb_prob."""
    if not cal_map:
        return raw_prob
    for b in cal_map:
...
def train_and_evaluate(use_historical=True):
    """Train Naive Bayes on all data, run walk-forward CV, save."""
    print("=" * 60)
    print(f"  Naive Bayes — Training {'(+historical)' if use_historical else '(graded only)'}")
    print(f"  GaussianNB, var_smoothing=1e-9")
    print("=" * 60)
    ...
    print("\n  Walk-Forward Cross-Validation (Naive Bayes):")
    folds, oof_probs, oof_actuals = walk_forward_cv(
        X, y, dates, sources=sources
    )

    if folds:
        avg_auc = np.mean([f['auc'] for f in folds])
        avg_acc = np.mean([f['accuracy'] for f in folds])
        print(f"\n  NB CV Summary: Avg AUC={avg_auc:.3f}, Avg Acc={avg_acc:.3f}")
    else:
        avg_auc = None
        avg_acc = None

    # Train final model on all data
    print(f"\n  Training final Naive Bayes on {len(y)} samples...")
    X_scaled, _, medians, scaler = _impute_and_scale(X)

    model = GaussianNB(var_smoothing=1e-9)
    model.fit(X_scaled, y)

    # Training set accuracy as a sanity check
    y_train_prob = model.predict_proba(X_scaled)[:, 1]
    y_train_pred = (y_train_prob >= 0.5).astype(int)
    train_acc = (y_train_pred == y).mean()
    train_auc = _compute_auc(y, y_train_prob)
    print(f"  Final train accuracy: {train_acc:.3f}, train AUC: {train_auc:.3f}")
    ...
    print(f"  Naive Bayes saved: {MODEL_PATH}")
    ...
    metadata = {
        ...
        'architecture': 'GaussianNB(var_smoothing=1e-9)',
    }
...
def score_props(results, model_path=None):
    """Load trained Naive Bayes and add nb_prob to each result dict."""
    if model_path is None:
        model_path = MODEL_PATH

    if not os.path.exists(model_path):
        raise FileNotFoundError(f"No trained model at {model_path}. Run: python3 nb_model.py train")
    ...
    for r, prob in zip(results, probs):
        ...
        r['nb_prob'] = raw
        if cal_map:
            r['nb_prob_calibrated'] = round(_calibrate_prob(raw, cal_map), 4)
...
def main():
    ...
    if command == 'train':
        ...
        if metadata:
            ...
            print(f"  Train Acc: {metadata['train_accuracy']:.3f}")

    elif command == 'eval':
        print("=" * 60)
        print(f"  Naive Bayes — Evaluation Only")
        print("=" * 60)
        ...
        sources = [r.get('_data_source', 'graded') for r in records]
        print("\n  Walk-Forward CV (Naive Bayes):")
        folds, _, _ = walk_forward_cv(X, y, dates, sources=sources)
        ...
        if len(sys.argv) < 3:
            print("Usage: python3 nb_model.py score <board_or_results.json>")
            sys.exit(1)
        ...
        scored = sum(1 for r in results if 'nb_prob' in r)
        print(f"  Scored {scored}/{len(results)} props")

        nb_probs = [r['nb_prob'] for r in results if 'nb_prob' in r]
        if nb_probs:
            arr = np.array(nb_probs)
            print(f"  nb_prob: mean={arr.mean():.3f}, std={arr.std():.3f}, "
