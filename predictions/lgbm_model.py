#!/usr/bin/env python3
"""
LightGBM Prop Classifier — Ensemble partner for XGBoost.

LightGBM gradient-boosted classifier. Handles NaN natively (no imputation needed).
Reuses exact same features as XGBoost (engineer_features from xgb_model.py).

Usage:
    python3 predictions/lgbm_model.py train          # Train on all data
    python3 predictions/lgbm_model.py eval           # Walk-forward CV
    python3 predictions/lgbm_model.py score <file>   # Score a board/results file
"""
...
try:
    ...
    print("ERROR: lightgbm not installed. Run: pip3 install lightgbm")
    sys.exit(1)
...
MODEL_PATH = os.path.join(PREDICTIONS_DIR, 'lgbm_model.json')
META_PATH = os.path.join(PREDICTIONS_DIR, 'lgbm_model_meta.json')

# Categorical feature indices (by name)
...
def _get_categorical_indices():
    """Get indices of categorical features in FEATURE_COLS."""
    indices = []
    for name in CATEGORICAL_FEATURES:
        if name in FEATURE_COLS:
            indices.append(FEATURE_COLS.index(name))
    return indices
...
def walk_forward_cv(X, y, dates, sample_weights=None, sources=None):
    ...
    for i in range(1, len(graded_dates)):
        ...
        # Cast categorical columns to int for LightGBM categorical support
        X_train_cat = X_train.copy()
        X_test_cat = X_test.copy()
        for idx in cat_indices:
            X_train_cat[:, idx] = np.nan_to_num(X_train_cat[:, idx], nan=-1).astype(int)
            X_test_cat[:, idx] = np.nan_to_num(X_test_cat[:, idx], nan=-1).astype(int)
...
def _calibrate_prob(raw_prob, cal_map):
    """Apply calibration map to a raw lgbm_prob."""
    if not cal_map:
        return raw_prob
    for b in cal_map:
...
def train_and_evaluate(use_historical=True):
    """Train LightGBM on all data, run walk-forward CV, save."""
    print("=" * 60)
    print(f"  LightGBM — Training {'(+historical)' if use_historical else '(graded only)'}")
    print(f"  n_estimators=500, max_depth=6, lr=0.05, leaves=31")
    print("=" * 60)
    ...
    print("\n  Walk-Forward Cross-Validation (LightGBM):")
    ...
    if folds:
        ...
        print(f"\n  LightGBM CV Summary: Avg AUC={avg_auc:.3f}, Avg Acc={avg_acc:.3f}")
    else:
        avg_auc = None
        avg_acc = None
    ...
    cat_indices = _get_categorical_indices()
    print(f"\n  Training final LightGBM on {len(y)} samples...")

    # Cast categoricals for final training
    X_final = X.copy()
    for idx in cat_indices:
        X_final[:, idx] = np.nan_to_num(X_final[:, idx], nan=-1).astype(int)
    ...
    for rank, idx in enumerate(indices[:20]):
        ...
        print(f"    {rank+1:2d}. {fname:35s} {importances[idx]}")
    ...
    metadata = {
        ...
        'architecture': 'LGBMClassifier(500, depth=6, lr=0.05, leaves=31)',
...
def score_props(results, model_path=None):
    """Load trained LightGBM and add lgbm_prob to each result dict."""
    if model_path is None:
        model_path = MODEL_PATH

    if not os.path.exists(model_path):
        raise FileNotFoundError(f"No trained model at {model_path}. Run: python3 lgbm_model.py train")

    # Load LightGBM booster from native format
    booster = lgb.Booster(model_file=model_path)

    # Load calibration map + categorical indices
    cal_map = None
    cat_indices = _get_categorical_indices()
    if os.path.exists(META_PATH):
        ...
        if 'categorical_indices' in meta:
            cat_indices = meta['categorical_indices']
    ...
    X_score = X.copy()
    for idx in cat_indices:
        if idx < X_score.shape[1]:
            X_score[:, idx] = np.nan_to_num(X_score[:, idx], nan=-1).astype(int)

    # Booster.predict returns probabilities directly
    probs = booster.predict(X_score)

    for r, prob in zip(results, probs):
        raw = round(float(prob), 4)
        r['lgbm_prob'] = raw
        if cal_map:
            r['lgbm_prob_calibrated'] = round(_calibrate_prob(raw, cal_map), 4)
...
def main():
    ...
    if command == 'train':
        ...
        print("=" * 60)
        print(f"  LightGBM — Evaluation Only")
        print("=" * 60)
        ...
        if len(sys.argv) < 3:
            print("Usage: python3 lgbm_model.py score <board_or_results.json>")
            sys.exit(1)
        ...
        lgbm_probs = [r['lgbm_prob'] for r in results if 'lgbm_prob' in r]
        if lgbm_probs:
            arr = np.array(lgbm_probs)
            print(f"  lgbm_prob: mean={arr.mean():.3f}, std={arr.std():.3f}, "
